...
<div>
        ...
        <div class="container">
            ...
            <div class="addr-list-wrap">
                ...
                <ul>
                    ...
                    <div class="addr-opration addr-set-default">
                        <a href="javascript:;" class="addr-set-default-btn" @click="setDeault(item.addressId)" v-if="!item.isDefault"><i>设为默认地址</i></a>
                    </div>
                    <div class="addr-opration addr-default" v-if="item.isDefault">默认地址</div>
                    </li>
                    <li class="addr-new" @click="mdShowAddress=true">
                ...
                <a class="addr-more-btn up-down-btn" href="javascript:;" @click="showMoreAddress" :class="{'open':limit > 3}">
                    更多
                    <i class="i-up-down" >
            ...
            <div class="order-foot-wrap">
                ...
                <div class="next-btn-wrap">
                    <router-link class="btn btn--m btn--red" to="/orderConfirm" :to="{path:'/orderConfirm',query:{'addressId':selectAddressId}}">下一步</router-link>
                </div>
            </div>
            </div>
        </div>
    ...
    <div class="md-modal modal-msg md-modal-transition md-show" v-if="mdShowAddress">
        <div class="md-modal-inner">
            <div class="md-top">
                <div class="md-title">
                    添加地址
                </div>
                <button class="md-close" @click="mdShowAddress=false">关闭</button>
            </div> 
            <div class="md-content">
                <div class="confirm-tips">>
                    <div class="error-wrap">
                            <span class="error error-show" v-show="error">请填写完整的地址信息</span>
                    </div> 
                    <ul>
                        <li class="regi_form_input">
                            <input type="text" tabindex="1" id="name" name="name" placeholder="name" data-type="loginname" class="regi_login_input regi_login_input_left" style="left:20px;width:350px;">
                        </li>
                        <li class="regi_form_input noMargin">
                            <input type="text" tabindex="2" id="postCode" name="postCode" placeholder="postCode" class="regi_login_input regi_login_input_left login-input-no input_text" 
                             style="left:20px;width:350px;">
                        </li>
                        <li class="regi_form_input noMargin">
                            <input type="text" tabindex="3" id="address" name="address" placeholder="address" class="regi_login_input regi_login_input_left login-input-no input_text" 
                             style="left:20px;width:350px;">
                        </li>
                        <li class="regi_form_input noMargin">
                            <input type="text" tabindex="4" id="telephone" name="telephone" placeholder="telephone" class="regi_login_input regi_login_input_left login-input-no input_text" 
                             style="left:20px;width:350px;">
                        </li>
                    </ul>
                </div> 
                <div class="btn-wrap">
                    <a class="btn btn--m" href="javascript:" @click="addAddress">确定</a>
                    <a class="btn btn--m" href="javascript:" @click="mdShowAddress=false">取消</a>    
                </div>
            </div>
        </div>
    </div>
...
export default {
  ...
  methods:{
      ...
      setDeault(addressId){
          let _this = this;
          axios.post('/users/setDefault',{addressId:addressId})
          .then((response)=>{
              let res = response.data;
              if(res.status == '0')
      ...
      addAddress(){
          ...
          if(name&&address&&telephone&&postCode){
            let r1=Math.floor(Math.random()*10);
            let r2=Math.floor(Math.random()*10);
            let addressId = r1+new Date().Format('yyyyMMddhhmmss')+r2;
            let addressItem = {
                    "isDefault" : false,
                    "tel" : telephone,
                    "postCode" : postCode,
